...
class Scanner {
    ...
    public Scanner(InputStream in, int blockSize) {
    ...
    public Scanner(String string, int blockSize) throws UnsupportedEncodingException {
        this.reader = new InputStreamReader(new ByteArrayInputStream((string + " ").getBytes("utf8")));
        this.blockSize = blockSize;
        readInput();
    }

    public Scanner(File file, String charset) throws FileNotFoundException, UnsupportedEncodingException {
        this(file, 1024, charset);
    }

    public Scanner(File file, int blockSize, String charset)
