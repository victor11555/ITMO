...
function check_expr (root){
    if(root instanceof Not){
        ...
        if(root.left instanceof And && (root.right instanceof Constant || root.right instanceof Variable || root.right instanceof Not)){
            let left = new Or(root.left.left, root.right);
            let right = new Or(root.left.right, root.right);
            root = new And(left, right);
        }
        else if(root.right instanceof And && (root.left instanceof Constant || root.left instanceof Variable|| root.left instanceof Not)){
            let left = new Or(root.left, root.right.left);
            let right = new Or(root.left, root.right.right,);
            root = new And(left, right);
        }
        root.left = check_expr(root.left);
        root.right = check_expr(root.right);
