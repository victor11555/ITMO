...
function check_expr (root){
    if(root instanceof Not){
        ...
        if(root.left instanceof Or && (root.right instanceof Constant || root.right instanceof Variable)){
            let left = new And(root.left.left, root.right);
            let right = new And(root.left.right, root.right);
            root = new Or(left, right);
        }
        else if(root.right instanceof Or && (root.left instanceof Constant || root.left instanceof Variable)){
            let left = new And(root.left, root.right.left);
            let right = new And(root.left, root.right.right,);
            root = new Or(left, right);
        }
        root.left = check_expr(root.left);
        root.right = check_expr(root.right);
